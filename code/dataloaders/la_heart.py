...
import h5py
from .utils import load_nrrd, zscore_norm
...
class LAHeart(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        if split=='train':
            ...
            with open('../data/train70.list', 'r') as f:
                self.image_list = f.readlines()
        elif split == 'val10':
            with open('../data/val10.list', 'r') as f:
                self.image_list = f.readlines()
        elif split == 'test20':
            with open('../data/test20.list', 'r') as f:
                self.image_list = f.readlines()
        elif split == 'test':
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._base_dir + "/" + image_name + "/mri_norm_roicrop.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        # image,_ = load_nrrd(os.path.join(self._base_dir,image_name,'lgemri.nrrd'))
        # image = zscore_norm(image)#0,1归一化
        # image = np.swapaxes(image, 2,0)
        # label,_ = load_nrrd(os.path.join(self._base_dir, image_name, 'laendo.nrrd'))
        # label[label>=1]=1#二值化
